...
if (isset($_GET['startdate'])) {
 ...
 if ($osd2 == NULL) {
$sql = "SELECT * FROM view_pickup WHERE pickup_complete=0 ORDER BY $sortline";
 }
else {
  $sql = "SELECT * FROM view_pickup WHERE pickup_date BETWEEN '$osd2' AND '$osd3' ORDER BY $sortline"; 
}
  ...
  echo '<h1>All Pickups from '. $reportdate->format('D, M jS, Y') . ' to ' . $reportdateend->format('D, M jS, Y') . '</h1>';
  
}

else {

// make a table showing list of pickups

echo '<h1>Current Pickups</h1>';

echo '<p>This is a list of all pickups that have not yet been completed</p>';
...
echo '<button type="submit" formmethod="get" formaction="pickupdisplay.php">Set Date Range</button></form>';

echo '<table border="1"><tr><th>ID</th><th>Category</th><th>Organization Name &amp; Address</th><th>Contact Phone & E-mail</th><th>Organization &amp; Food Drive Notes</th><th>Pickup Date</th><th>Staff</th><th>Items Required</th><th>Pounds Received</th><th>Action</th></tr>';

// run through each pickup

$curdate = new DateTime();

while ($result2 = $result->fetch_assoc()) {

  echo '<tr><td>' . $result2["pickup_id"] . '</td><td>' . $result2["orgcat_name"] . '</td><td>' . $result2["org_name"] . '<br>' . $result2["org_street"] . '<br>' . $result2["org_city"] . ', ' . $result2["org_state"] . ' ' . $result2["org_zip"] . '</td><td>' . $result2["org_contact"] . '<br>' . $result2["org_phone"] . '<br>' . '<a href="mailto:' . $result2["org_email"] . '">' . $result2["org_email"] . '</a></td><td>';

  echo nl2br($result2['org_notes']);
  echo '<p>';  
  echo nl2br($result2['fooddrive_notes']);
  
  $startdate = new DateTime($result2["pickup_date"]);
  ...
  $sql2 = "SELECT * from view_pickupitemexpanded WHERE pickup_id=$result2[pickup_id]";
  ...
  echo '<td>' . $result2["pickup_pounds"] . '</td>';
  
  echo '<td><form><input type="hidden" name="pickupid" value="' . $result2['pickup_id'] . '">';
